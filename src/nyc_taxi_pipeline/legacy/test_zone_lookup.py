...
# 🌟 修复 1：导入路径改为重构后的 zone_lookup
from nyc_taxi_pipeline.spatial.zone_lookup import (
...
def test_process_dim_taxi_zone(mock_spatial_data):
    """测试维度表生成逻辑"""
    # 🌟 修复 2：同步源码变化，传入 resolution 参数（例如 8）
    pdf_dim = process_dim_taxi_zone(mock_spatial_data, resolution=8)
...
def test_process_bridge_taxi_zone_h3(mock_spatial_data):
    ...
    # 🌟 修复 2：同步源码变化，传入 resolution 参数（例如 8）
    pdf_bridge = process_bridge_taxi_zone_h3(mock_spatial_data, resolution=8)
